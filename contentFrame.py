...
class ContentFrame(wx.Frame):
    ...
    def InitUI(self):
        ...
        self.count = 0
        self.ThreadNums = 1   # 线程数
        self.proxyIp = None   # 代理
        self.LinkHeaderCountry = 'fr'  #国家
        self.pathVerify = path.join(getcwd(), 'ca.crt')  # 证书
        self.InfoQue = queue.Queue()   # 详情队列
        self.LinkQue = queue.Queue()   # 链接队列
        self.currentInfoFilePath = None   # 详情文件路径
        self.currentLinkFilePath = None   # 链接文件路径
        self.ThreadSwitch = 1  # 线程状态
...
class myThread(threading.Thread):
    def __init__(self, threadID, name, counter, window, statustype):
        ...
        self.name = name
        self.data = None
        self.autiZipDir = path.join(getcwd(), 'anticaptcha-plugin_v0.48.zip')   # 插件路径
    ...
    def run(self):
        if self.statusType == 'Info':
            while self.window.InfoQue.qsize():
                ...
                # 捉取详情页
                infoResult = inforeq.infostart(infourl, self.window.proxyIp, self.window.pathVerify,
                                               self.window.LinkHeaderCountry)
                print(infoResult)
                if infoResult == -1:
            ...
            while self.window.LinkQue.qsize():
                ...
                linkResult, linkList = linkreq.Linkstatus(linkurl, self.window.proxyIp, self.window.pathVerify,
                                                          self.window.currentTempLinkFilePath)
                if len(linkList) > 0:
